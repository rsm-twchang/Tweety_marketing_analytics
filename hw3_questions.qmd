...
The maximum likelihood estimation converged successfully using the BFGS algorithm. The model required 13 iterations and 90 function evaluations. All four parameters were estimated with high precision, as indicated by relatively small standard errors. The log-likelihood value at convergence was 879.86.
	•	brand_N and brand_P have positive coefficients, confirming that consumers prefer Netflix and Prime over the baseline (Hulu).
	•	The negative coefficient for ad_yes shows that consumers are less likely to choose an option with ads.
	•	The negative price coefficient reflects standard price sensitivity higher prices reduce utility.

All 95% confidence intervals exclude zero, supporting the statistical significance of each effect.
...
The Bayesian estimation using Metropolis-Hastings produced stable posterior estimates. All four parameters have posterior means consistent with prior expectations and previous MLE results. Consumers show stronger preference for Netflix over Prime, dislike ads, and are price-sensitive. The negative value for price confirms that higher prices reduce utility. Posterior standard deviations are small, indicating precise estimates. The 95% credible intervals do not cross zero, supporting the significance of all effects.
